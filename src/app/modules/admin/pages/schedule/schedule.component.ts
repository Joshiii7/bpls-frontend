...
export class ScheduleComponent {
  ...
  ngOnInit() {
    ...
    this.scheduleForm.get('scheduleType')!.valueChanges.subscribe(value => this.syncScheduleOtherValidator(value));
  }

  get showScheduleTypeOtherField(): boolean {
    return this.scheduleForm.get('scheduleType')?.value === 'others';
  }

  private syncScheduleOtherValidator(scheduleType: string): void {
    const otherControl = this.scheduleForm.get('scheduleTypeOther')!;
  ...
  addSchedule() {
    ...
    if (this.scheduleForm.invalid || !this.startDate || !this.endDate) {
      Swal.fire({
        icon: 'error',
        title: 'Oops...',
        text: this.showScheduleTypeOtherField && this.scheduleForm.get('scheduleTypeOther')?.invalid
          ? 'Please specify what this schedule is for.'
    ...
    const newSchedule: any = {
      schedule_type: SCHEDULE_TYPE_LABELS[this.scheduleForm.value.scheduleType] || this.scheduleForm.value.scheduleType,
      schedule_type_other: this.showScheduleTypeOtherField ? this.scheduleForm.value.scheduleTypeOther : null,
